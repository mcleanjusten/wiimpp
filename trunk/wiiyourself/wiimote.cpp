...
//
//	 - WiiYourself! - native C++ Wiimote library  v1.13 BETA
...
inline void wiimote::Reset ()
	...
	MuteSpeaker  (true);
	EnableSpeaker(false);
	Sleep(120); // avoids loosing the extension calibration data on Connect()
	}
// ------------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------------
void wiimote::InitializeMotionPlusExtension ()
	{
	// Initialize it:
	...
	//  and detect it as a standard extension
...
int wiimote::ParseStatus (BYTE* buff)
	...
	if(extension)
		...
		// no ordinary extension is currently connected, see if a MotionPlus
		//  is plugged in (it doesn't report as a normal extension until activateD)
		InitializeMotionPlusExtension();
	...
	switch(Internal.ExtensionType)
		...
		case BALANCE_BOARD:
			...
			Internal.BalanceBoard.Kg.Total	 = (Internal.BalanceBoard.Kg.TopL    +
												Internal.BalanceBoard.Kg.TopR    +
												Internal.BalanceBoard.Kg.BottomL +
												Internal.BalanceBoard.Kg.BottomR) / 4.f;
			// and convert to Lbs
		...
		case MOTION_PLUS:
			{
      // CWiid implementation (http://wiibrew.org/wiki/Motion_Plus#Wii_Motion_Plus)
      wiimote_state::motion_plus::sensors_raw prev_raw =
														Internal.MotionPlus.Raw;
      Internal.MotionPlus.Raw.Yaw = ((unsigned short)buff[offset+3] & 0xFC)<<6 |
                                     (unsigned short)buff[offset+0];
      Internal.MotionPlus.Raw.Pitch = ((unsigned short)buff[offset+5] & 0xFC)<<6 |
                                       (unsigned short)buff[offset+2];
      Internal.MotionPlus.Raw.Roll = ((unsigned short)buff[offset+4] & 0xFC)<<6 |
                                      (unsigned short)buff[offset+1];
			if((Internal.MotionPlus.Raw.Yaw   != prev_raw.Yaw  ) ||
			   (Internal.MotionPlus.Raw.Pitch != prev_raw.Pitch) ||
			   (Internal.MotionPlus.Raw.Roll  != prev_raw.Roll ))
				changed |= MOTIONPLUS_SPEED_CHANGED;

      float yFact = buff[offset+3] & 0x2 ? 0.0505f : 0.23f;
      float pFact = buff[offset+3] & 0x1 ? 0.0505f : 0.23f;
      float rFact = buff[offset+4] & 0x2 ? 0.0505f : 0.23f;

      Internal.MotionPlus.Speed.Yaw   = -(float)(Internal.MotionPlus.Raw.Yaw-0x1F7F)*yFact;
			Internal.MotionPlus.Speed.Pitch = -(float)(Internal.MotionPlus.Raw.Pitch-0x1F7F)*pFact;
			Internal.MotionPlus.Speed.Roll  = -(float)(Internal.MotionPlus.Raw.Roll-0x1F7F)*rFact;

      /* original WiiYourself implementation
      wiimote_state::motion_plus::sensors_raw prev_raw =
														Internal.MotionPlus.Raw;
			Internal.MotionPlus.Raw.Yaw   = (short)((short)buff[offset+3] << 9 |
														   buff[offset+0] << 1);
			Internal.MotionPlus.Raw.Roll  = (short)((short)buff[offset+4] << 9 |
														   buff[offset+1] << 2);
			Internal.MotionPlus.Raw.Pitch = (short)((short)buff[offset+5] << 9 |
														   buff[offset+2] << 1);
			if((Internal.MotionPlus.Raw.Yaw   != prev_raw.Yaw  ) ||
			   (Internal.MotionPlus.Raw.Pitch != prev_raw.Pitch) ||
			   (Internal.MotionPlus.Raw.Roll  != prev_raw.Roll ))
			...
			Internal.MotionPlus.Speed.Yaw   = (float)-Internal.MotionPlus.Raw.Yaw   /
												(1<<15);
			Internal.MotionPlus.Speed.Pitch = (float)-Internal.MotionPlus.Raw.Pitch /
												(1<<15);
			Internal.MotionPlus.Speed.Roll  = (float)-Internal.MotionPlus.Raw.Roll  /
												(1<<15);
      */
			}
			break;
...
int wiimote::ParseReadAddress (BYTE* buff)
	...
	if((buff[3] & 0x08) != 0) {
		...
		// this will happen when attempting to detect a MotionPlus that isn't
		//  connected
...
//		_ASSERT(0);
	...
	switch(address)
		...
		case (REGISTER_EXTENSION_TYPE & 0xffff):
			{
			_ASSERT(size == 5);
			unsigned __int64 type = *(unsigned __int64*)buff;

			static const unsigned __int64 NUNCHUK		     = 0x000020A40000;
			static const unsigned __int64 CLASSIC		     = 0x000020A40101;
			static const unsigned __int64 GH3_GHWT_GUITAR    = 0x000020A40301;
			static const unsigned __int64 GHWT_DRUMS	     = 0x000120A40301;
			static const unsigned __int64 BALANCE_BOARD	     = 0x020420A40000;
			static const unsigned __int64 MOTION_PLUS		 = 0x050420A40000;
			static const unsigned __int64 MOTION_PLUS_DETECT = 0x050020a60000;
			...
			else if(type == MOTION_PLUS_DETECT)
				{
				bMotionPlus = true;
				// Initialize it - this maps it to the standard extension port
				InitializeMotionPlusExtension();
				break;
				}
			// MotionPlus: once it's activated & mapped to the standard ext. port
			else IF_TYPE(MOTION_PLUS)
...
//					((Internal.ExtensionType != BALANCE_BOARD) && (size == 15)));
			...
			switch(Internal.ExtensionType)
				...
				case BALANCE_BOARD:
					...
					Internal.BalanceBoard.CalibrationInfo.Kg17.TopR	   =
									(short)((short)buff[8] << 8 | buff[9]);
					Internal.BalanceBoard.CalibrationInfo.Kg17.BottomR =
									(short)((short)buff[10] << 8 | buff[11]);
					Internal.BalanceBoard.CalibrationInfo.Kg17.TopL	   =
									(short)((short)buff[12] << 8 | buff[13]);
					Internal.BalanceBoard.CalibrationInfo.Kg17.BottomL =
